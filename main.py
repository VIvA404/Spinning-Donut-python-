'''Пишем пончик на петухоне с помощью WLS в первое раз'''
import pygame as pg
import math
import colorsys
...
theta_spacing = 10
# мы изменим его phi_spacing позже для более быстрого вращения
phi_spacing = 5
...
while run:
    ...
    for j in range(0, 628, theta_spacing):  # 0 to 2pi
        for i in range(0, 628, phi_spacing):
            ...
            n = math.sin(B)
            t = c * h * g - f * e
            # Это для координаты 3D x после вращения
            x = int(x_offset + 40 * D * (l * h * m - t * n))
            # Это для 3D-координаты y после вращения
            y = int(y_offset + 20 * D * (l * h * n + t * m))
            o = int(x + columns * y)
            N = int(8 * ((f * e - c * d * g) * m - c * d * e - f * g - l * d * n))  # индека яркости
    ...
    for i in range(len(b)):
        #Для более быстрого вращения измените значение на более высокое.
        A += 0.00004
        B += 0.00002
        if i == 0 or i % columns:
